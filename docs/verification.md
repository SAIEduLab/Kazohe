...
スクリーンショットの作成は目視合格ではありません。筆算の位、借り越し、小数点、部分積、商の0、入力欄の欠け、説明の折返し、学年に合う言葉、選択内容と支援の分かりやすさを実画像で確認し、対象と確認者を `tests/evidence/visual-review.json` に記録します。現在の新Public用記録は未確認項目を `NOT_RUN` としています。
